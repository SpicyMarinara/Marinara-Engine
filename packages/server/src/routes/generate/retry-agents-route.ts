...
type SecretPlotDirection = { direction: string; fulfilled?: boolean };
...
function normalizeSecretPlotSceneDirections(raw: unknown): SecretPlotDirection[] {
  if (!Array.isArray(raw)) return [];
...
async function executeRetryBatches(
  ...
  for (const entry of resolvedAgents) {
    ...
    const key = `${entry.agentProvider.constructor.name}::${entry.agentModel}::${contextKind}`;
    ...
    if (retryMessageId && result.success && (result.type === "context_injection" || result.type === "director_event")) {
      ...
      try {
        ...
        if (msg) {
          ...
          if (text.trim()) {
            const entry = { agentType: result.agentType, text };
